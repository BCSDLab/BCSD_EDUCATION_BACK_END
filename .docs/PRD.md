...
- 종류: FrontEnd, BackEnd, Android, iOS, Design, Data Analyst, Game, Product Manager (총 8개)
- 내부 Enum 값은 `FRONTEND`, `BACKEND`, `ANDROID`, `IOS`, `DESIGN`, `DATA_ANALYST`, `GAME`, `PRODUCT_MANAGER`를 사용
- 트랙별로 커리큘럼, 강의 자료, 출석이 독립적으로 운영
- 특정 기수에 특정 트랙이 모집되지 않을 수 있음 (예: 25-상반기에 FrontEnd 미모집)
...
- **가입 방식**: 초대 기반
  - 교육장이 대상 이메일로 초대 메일을 발송
  - 초대 생성 시 이메일, 이름, 기수-트랙을 함께 고정 지정
  - 메일의 1회성 초대 링크를 통해 회원가입 페이지로 진입
  - 링크 검증이 완료되면 이메일, 이름, 트랙, 기수는 자동 입력되며 수정할 수 없음
- **회원가입 입력 필드**
  - 이메일 (초대 링크 기반 자동 입력, read-only)
  - 트랙 (`FrontEnd`, `BackEnd`, `Android`, `iOS`, `Design`, `Data Analyst`, `Game`, `Product Manager`) (초대 기반 자동 입력, read-only)
  - 기수 (`xx-상반기`, `xx-하반기`) (초대 기반 자동 입력, read-only)
  - 이름 (초대 기반 자동 입력, read-only)
  - 비밀번호 / 비밀번호 확인
  - 운영 규정 및 개인정보처리방침 통합 동의 체크
- **검증 규칙**
  - 이메일은 올바른 형식이어야 함
  - 비밀번호는 8자 이상이며 영문, 숫자, 특수문자를 모두 포함해야 함
  - 비밀번호 확인 값은 비밀번호와 일치해야 함
  - 운영 규정 및 개인정보처리방침 통합 동의는 필수
- **초대 상태 관리**
  - 초대는 `PENDING`, `ACCEPTED`, `EXPIRED`, `CANCELLED` 상태를 가짐
  - 초대 링크는 1회만 사용할 수 있으며 기본 만료 시간은 72시간으로 설정
  - 만료 또는 취소된 초대는 재초대 후 다시 가입할 수 있음

#### 4.1.2 로그인/로그아웃

- 이메일 + 비밀번호 기반 로그인
- JWT 토큰 발급 (Access Token + Refresh Token)
- 가입이 완료되지 않았거나, 초대가 만료되었거나, 비활성화된 계정은 로그인할 수 없음
- 로그아웃 시 토큰 무효화
...
- 운영 규정 동의 시각, 개인정보처리방침 동의 시각
- 초기 회원가입 시 1개의 기수-트랙 소속을 생성하고, 이후 추가 소속은 관리 페이지에서 부여
...
- **로그인 페이지**: 이메일/비밀번호 입력
- **회원가입 페이지**: 초대 토큰 검증, 이메일/트랙/기수/이름 자동 입력(read-only), 비밀번호/약관 동의 입력
...
| agreed_terms_at   | DateTime | 운영 규정 동의 일시                              |
| agreed_privacy_at | DateTime | 개인정보처리방침 동의 일시                       |
...
| semester_track_id | Long    | FK → SemesterTrack (고정 기수-트랙)       |
| token_hash       | String   | 초대 토큰 해시                            |
| status           | Enum     | PENDING, ACCEPTED, EXPIRED, CANCELLED     |
| expires_at       | DateTime | 만료 일시                                 |
| invited_by       | Long     | FK → User (초대한 교육장)                 |
| accepted_user_id | Long     | FK → User (가입 완료 사용자, nullable)    |
| created_at       | DateTime | 생성일                                    |
| accepted_at      | DateTime | 수락일시 (nullable)                       |
...
| id         | Long    | PK                            |
| year       | Integer | 연도                          |
| half       | Enum    | FIRST_HALF(상반기), SECOND_HALF(하반기) |
| start_date | Date    | 시작일                        |
| end_date   | Date    | 종료일                        |
...
| ---- | ------ | --------------------------------------------------------------------------- |
| id   | Long   | PK                                                                          |
| code | Enum   | 내부 코드 (`FRONTEND`, `BACKEND`, `ANDROID`, `IOS`, `DESIGN`, `DATA_ANALYST`, `GAME`, `PRODUCT_MANAGER`) |
| name | String | 표시명 (FrontEnd, BackEnd, Android, iOS, Design, Data Analyst, Game, Product Manager) |
...
| GET    | /api/auth/invitations/{token}    | 초대 토큰 검증 및 가입 정보 조회 |
...
- 비밀번호는 Node.js `crypto.scrypt` 기반 해시로 저장
- JWT 기반 인증 (Access Token: 30분, Refresh Token: 7일)
- 초대 토큰과 비밀번호 재설정 토큰은 원문이 아닌 해시 값으로 저장
- 이메일 형식 검증
- API 권한 검증 (역할 기반)
...
- [x] NestJS 프로젝트 초기화 (pnpm, 모듈 구조)
- [x] PostgreSQL 연동 및 Prisma 설정 (멀티 파일 스키마, 첫 마이그레이션)
- [x] `PrismaModule` / `PrismaService` 작성 및 전역 주입 설정
- [x] `ConfigModule` 전역 등록 (`@nestjs/config`, `.env` 관리)
- [x] 공통 응답 포맷 Interceptor + 전역 `ExceptionFilter` 구성
- [x] `ValidationPipe` 전역 등록 (`class-validator` / `class-transformer`)
- [ ] `@nestjs/passport` + `@nestjs/jwt` 기반 인증/인가 Guard 구현
- [ ] AWS S3 연동 설정 (`@aws-sdk/client-s3`, 파일 업로드/다운로드 공통 서비스)
- [x] User 엔티티(Prisma 모델) 및 Role Enum 정의
- [ ] AWS SES 연동 이메일 발송 서비스 구현 (`@aws-sdk/client-ses`, 초대·비밀번호 재설정)
- [ ] 인증/메일 발송용 환경변수 추가 (`JWT_*`, `FRONTEND_BASE_URL`, `AWS_*`)
...
- [ ] 초대 토큰 검증 API (`GET /api/auth/invitations/{token}`) — 자동 입력용 이메일/이름/기수-트랙 조회
- [ ] 회원 초대 API (`POST /api/auth/invite`) — 초대 토큰 생성, 고정 이름/기수-트랙 지정, 이메일 발송
- [ ] 초대 기반 회원가입 API (`POST /api/auth/register`) — 토큰 검증, 고정 초대 정보 검증, 비밀번호 정책 검증, 약관 동의 기록
...
- [ ] 초대 기반 회원가입 페이지 (이메일/트랙/기수/이름 read-only, 비밀번호/통합 동의 체크)
- [ ] 초대 만료/잘못된 링크 안내 화면
- [ ] 비밀번호 찾기 페이지 (이메일 입력 → 재설정 링크 발송)
- [ ] 비밀번호 재설정 페이지 (토큰 기반)
- [ ] 마이페이지 (내 정보 조회/수정)
- [ ] 회원 관리 페이지 — 교육장 전용 (목록, 역할 변경, 초대 발송/재발송, 상태 확인)
...
- [ ] Track 엔티티 및 조회 API (8개 트랙 초기 데이터)
