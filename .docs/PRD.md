...
- **가입 방식**: 초대 기반
  - 교육장이 한국기술교육대학교 이메일로 초대 메일을 발송
  - 메일의 1회성 초대 링크를 통해 회원가입 페이지로 진입
  - 링크 검증이 완료되면 이메일은 자동 입력되며 수정할 수 없음
- **회원가입 입력 필드**
  - 이메일 (초대 링크 기반 자동 입력, read-only)
  - 트랙 (`FrontEnd`, `BackEnd`, `Android`, `iOS`, `Design`, `Data Analyst`, `Game`, `Product Manager`)
  - 기수 (`xx-상반기`, `xx-하반기`)
  - 이름
  - 비밀번호 / 비밀번호 확인
  - 운영 규정 동의 체크
...
- **로그인 페이지**: 이메일/비밀번호 입력
- **회원가입 페이지**: 초대 토큰 검증, 이메일 자동 입력, 트랙/기수/이름/비밀번호/약관 동의 입력
...
| ---- | ------ | --------------------------------------------------------------------------- |
| id   | Long   | PK                                                                          |
| name | String | 트랙명 (FrontEnd, BackEnd, Android, iOS, Design, Data Analyst, Game, Product Manager) |
...
- [ ] 초대 토큰 검증 API (`GET /api/auth/invitations/{token}`) — 자동 입력용 이메일 조회
- [ ] 회원 초대 API (`POST /api/auth/invite`) — 초대 토큰 생성, 이메일 발송
- [ ] 초대 기반 회원가입 API (`POST /api/auth/register`) — 토큰 검증, 도메인 검증, 비밀번호 정책 검증, 약관 동의 기록
...
- [ ] 초대 기반 회원가입 페이지 (이메일 read-only, 트랙/기수/이름/비밀번호/동의 체크)
- [ ] 초대 만료/잘못된 링크 안내 화면
- [ ] 비밀번호 찾기 페이지 (이메일 입력 → 재설정 링크 발송)
- [ ] 비밀번호 재설정 페이지 (토큰 기반)
- [ ] 마이페이지 (내 정보 조회/수정)
